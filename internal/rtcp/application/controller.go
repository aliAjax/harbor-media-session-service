...
// Controller caches the latest RTCP reports and throttles keyframe requests.
// A single controller is shared by the RTCP ingest path and the cleanup path
// of one media session, so every accessor must hold the lock.
